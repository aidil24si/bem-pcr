                          ...
                          <Card key={a.id} onClick={() => toggleAspirationSelection(a.id)} className={`border-gray-200 bg-white cursor-pointer transition-all shadow-sm ${isSelected ? 'ring-2 ring-[#004B5F] border-[#004B5F] bg-[#E6F3F7]/50' : 'hover:border-[#004B5F]/40 hover:shadow-md'}`}>
                            ...
                            <CardContent className="space-y-4 pt-4">
                              ...
                              <div className="flex justify-between items-center mt-2">
                                ...
                                <button 
                                  onClick={(e) => { e.stopPropagation(); setViewAspirasi(a); }}
                                  className="text-[11px] font-bold text-[#004B5F] bg-[#E6F3F7] hover:bg-[#CCE7EF] px-3 py-1.5 rounded-lg transition-colors"
                                >
                                  Lihat Detail
                                </button>
                              </div>
                        ...
                        <DialogHeader>
                          ...
                          <DialogDescription className="text-slate-500 flex flex-wrap items-center gap-2 mt-2">
                            <span className="px-2.5 py-1 bg-slate-100 text-slate-600 rounded-md text-[10px] font-bold uppercase tracking-wider">
                              {viewAspirasi.tipe_isu === 'tangible' ? 'Fasilitas' : viewAspirasi.tipe_isu === 'intangible' ? 'Birokrasi' : viewAspirasi.tipe_isu}
                            </span>
                            <span className="text-slate-300">•</span>
                            <span className="text-xs">{new Date(viewAspirasi.created_at).toLocaleString('id-ID')}</span>
                          </DialogDescription>
                        </DialogHeader>
                        
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-gray-100">
                          <div>
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Pengirim</label>
                            <p className="font-semibold text-slate-700 text-sm mt-1">
                              {viewAspirasi.is_anonim ? 'Anonim' : (viewAspirasi.identitas?.nama || 'Anonim')}
                            </p>
                          </div>
                          <div>
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">NIM</label>
                            <p className="font-semibold text-slate-700 text-sm mt-1">
                              {viewAspirasi.is_anonim ? '-' : (viewAspirasi.identitas?.nim || '-')}
                            </p>
                          </div>
                          {(!viewAspirasi.is_anonim && viewAspirasi.identitas?.email) && (
                            <div className="sm:col-span-2">
                              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Email</label>
                              <p className="font-semibold text-slate-700 text-sm mt-1">{viewAspirasi.identitas.email}</p>
                            </div>
                          )}
                          <div className="sm:col-span-2">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Kategori Detail</label>
                            <p className="font-semibold text-slate-700 text-sm mt-1">{viewAspirasi.kategori}</p>
                          </div>
                          <div className="sm:col-span-2">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Status</label>
                            <p className="font-semibold text-slate-700 text-sm mt-1">{viewAspirasi.status || (viewAspirasi.rilis_id ? 'Terkonsolidasi' : 'Menunggu')}</p>
                          </div>
                        </div>

                        <div>
                          <label className="text-[10px] font-bold text-[#004B5F] uppercase tracking-wider flex items-center gap-2 mb-2">Deskripsi Aspirasi</label>
                          <div className="bg-white border border-gray-200 p-4 rounded-xl shadow-sm">
                            <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{viewAspirasi.deskripsi}</p>
                          </div>
                        </div>
